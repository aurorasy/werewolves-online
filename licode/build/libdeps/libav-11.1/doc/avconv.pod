...
avconv [global options] [[infile options][B<-i> I<infile>]]... {[outfile options] I<outfile>}...
...
The transcoding process in B<avconv> for each output can be described by
...
B<avconv> calls the libavformat library (containing demuxers) to read
input files and get packets containing encoded data from them. When there are
multiple input files, B<avconv> tries to keep them synchronized by
...
Before encoding, B<avconv> can process raw audio and video frames using
filters from the libavfilter library. Several chained filters form a filter
graph.  B<avconv> distinguishes between two types of filtergraphs -
simple and complex.
...
B<-codec> option. It makes B<avconv> omit the decoding and encoding
...
B<avconv> will seek to the closest seek point before I<position>.
...
The format is B<wxh> (default - same as source).  The following
...
faster than software decoding on modern CPUs. Additionally, B<avconv>
...
Exit after avconv has been running for I<duration> seconds.
...
F<presets> directory in the Libav source tree for examples.

Preset files are specified with the C<pre> option, this option takes a
preset name as input.  Avconv searches for a file named I<preset_name>.avpreset in
the directories F<$AVCONV_DATADIR> (if set), and F<$HOME/.avconv>, and in
the data directory defined at configuration time (usually F<$PREFIX/share/avconv>)
...
NOTE: To see the supported input formats, use C<avconv -formats>.
...
The option C<-decoders> of the av* tools will display the list of
enabled decoders.
...
The option C<-encoders> of the av* tools will display the list of
enabled encoders.
...
Libav C<b> option is expressed in bits/s, x264 C<bitrate> in kilobits/s.
...
=item B<-bluray-compat> I<integer>
...
The option C<-bsfs> of the av* tools will display the list of
...
options in B<avconv> and B<-vf> in B<avplay>, and by the
...
The expression can contain the constants I<PI>, I<E>, I<PHI>, I<AVTB> (the
default timebase), I<intb> (the input timebase), and I<sr> (the sample rate,
audio only).

The default value for the input is I<intb>.
...
the following syntax: C<x0/y0|x1/y1|x2/y2|....>
...
whenever conversion is needed. Use the I<aformat> filter to force a specific
conversion.
...
I<sample_rate> specifies the sample rate, and defaults to 44100.

I<channel_layout> specifies the channel layout, and can be either an
...
The name of the sample format, as returned by C<av_get_sample_fmt_name()>.
...
configure Libav with C<--enable-libfontconfig>.
...
These are the same as I<main_w> and I<main_h>.


=item B<text_w, text_h>
...
These are the same as I<text_w> and I<text_h>.
...
numbers between 0.0 and 1.0, inclusive) or by an C<av_parse_color()> color
description), a position (specified as I<X>/I<Y>, where
...
For example, to horizontally flip the input video with B<avconv>:
...
These are the same as I<overlay_w> and I<overlay_h>.
...
The input display aspect ratio, same as I<iw> / I<ih>.
...
The parameters I<w> and I<h> are expressions containing
...
The default value of I<w> and I<h> is 0.
...
The parameter I<dar> is an expression containing
the following constants:
...
B<avconv>; the B<-filter_complex> option fully replaces it.
...
I<width>:I<height>:I<timebase> as an optional parameter.
...
Also see the the C<av_parse_time()> function.
